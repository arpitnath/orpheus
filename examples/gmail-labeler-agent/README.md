...
A production-grade AI agent that automatically labels Gmail messages using OpenAI Agents SDK and Composio.
...
### 4. Deploy to AgentScale

```bash
cd ../..  # Back to agentscale root
agentscale deploy examples/gmail-labeler-agent --remote
```
...
```
gmail-labeler-agent/
├── agent.yaml              # AgentScale deployment config
├── requirements.txt        # Pinned Python dependencies
├── .env.example           # Environment variable template
├── .gitignore             # Ignore sensitive data
│
├── src/
│   ├── handler.py         # AgentScale entry point
│   │
│   ├── agent/
...
#### List Labels
```bash
curl -X POST http://localhost:7777/v1/agents/gmail-labeler/run \
...
This is an example agent for AgentScale. Feel free to:
- Fork and customize
- Report issues
...
- [AgentScale](https://github.com/yourusername/agentscale)
