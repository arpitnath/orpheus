...
## Deploy to AgentScale

```bash
# Build Node.js base image (first time only)
./scripts/build-nodejs20-from-lima.sh

# Deploy the agent
agentscale deploy ./examples/calculator-agent-js

# Invoke the agent
curl -X POST http://localhost:7654/v1/agents/calculator-agent-js/run \
...
AgentScale expects an async `handler` function that takes input data and returns a result:
