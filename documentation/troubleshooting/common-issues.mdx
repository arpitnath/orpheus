...
orpheus runs <agent> --status STARTED
...
orpheus runs <agent> --limit 100 | grep duration
```
...
orpheus logs <agent> | grep -i "permission\|eperm"
```
...
orpheus logs daemon | grep "circuit breaker"
orpheus logs daemon | grep "spawn failed"
```
...
- Daemon logs: `orpheus logs daemon | tail -50`
