...
const DefaultInstanceName = "orpheus"
...
type Manager struct {
	InstanceName string
	TemplatePath string // Path to orpheus.yaml template
}
...
// The socket is forwarded from /var/run/orpheus.sock inside the VM
// to ~/.lima/{instance}/sock/orpheus.sock on the host.
func (m *Manager) SocketPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".lima", m.InstanceName, "sock", "orpheus.sock")
}
