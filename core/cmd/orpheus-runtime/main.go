// orpheus-runtime is the Go runtime binary for executing agents.
package main
...
func printUsage() {
	fmt.Println(`orpheus-runtime - Agent execution runtime

Usage:
  orpheus-runtime run [options] <agent-dir>
  orpheus-runtime version
  orpheus-runtime help
...
Examples:
  echo '{"query": "hello"}' | orpheus-runtime run --no-isolate ./my-agent
  orpheus-runtime run --timeout 120 --no-isolate ./my-agent`)
}
...
func runCmd(args []string) {
	...
	if fs.NArg() < 1 {
		...
		fmt.Fprintln(os.Stderr, "Usage: orpheus-runtime run <agent-dir> [options]")
		os.Exit(1)
	}
	agentDir := fs.Arg(0)
